        ...
        Commands::Version => {
            ...
            match release_channel {
                ReleaseChannel::Stable | ReleaseChannel::Preview => {
                    println!("{}", env!("PRISM_PKG_VERSION"))
                }
                ReleaseChannel::Nightly | ReleaseChannel::Dev => {
                    let commit_sha =
                        option_env!("PRISM_COMMIT_SHA").unwrap_or(release_channel.dev_name());
                    let build_id = option_env!("PRISM_BUILD_ID");
...
pub static VERSION: LazyLock<String> = LazyLock::new(|| match *RELEASE_CHANNEL {
    ReleaseChannel::Stable | ReleaseChannel::Preview => env!("PRISM_PKG_VERSION").to_owned(),
    ReleaseChannel::Nightly | ReleaseChannel::Dev => {
        let commit_sha = option_env!("PRISM_COMMIT_SHA").unwrap_or("missing-zed-commit-sha");
        let build_identifier = option_env!("PRISM_BUILD_ID");
...
fn is_new_version(version: &str) -> bool {
    semver::Version::from_str(version)
        .ok()
        .zip(semver::Version::from_str(env!("PRISM_PKG_VERSION")).ok())
