...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("PrisM.exe"), p("old\\PrisM.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\PrisM.exe"), p("old\\bin\\PrisM.exe")),
        ...
        // Copy new files
        Job::move_file(p("install\\PrisM.exe"), p("PrisM.exe")),
        Job::move_file(p("install\\bin\\PrisM.exe"), p("bin\\PrisM.exe")),
...
fn release_file_handles(app_dir: &Path) -> Result<()> {
    ...
    let files_to_release = [
        app_dir.join("PrisM.exe"),
        app_dir.join("bin\\PrisM.exe"),
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>, launch: bool) -> Result<()> {
    ...
    if launch {
        ...
        let _ = std::process::Command::new(app_dir.join("PrisM.exe")).spawn();
    }
