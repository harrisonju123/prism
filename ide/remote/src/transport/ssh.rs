...
#[cfg(windows)]
impl MasterProcess {
    const CONNECTION_ESTABLISHED_MAGIC: &str = "PRISM_SSH_CONNECTION_ESTABLISHED";
...
impl RemoteConnection for SshRemoteConnection {
    ...
    fn start_proxy(
        ...
        const VARS: [&str; 3] = ["RUST_LOG", "RUST_BACKTRACE", "PRISM_GENERATE_MINIDUMPS"];
...
impl SshRemoteConnection {
    ...
    async fn ensure_server_binary(
        ...
        let wanted_version = cx.update(|cx| match release_channel {
            ...
            ReleaseChannel::Dev => {
                anyhow::bail!(
                    "PRISM_BUILD_REMOTE_SERVER is not set and no remote server exists at ({:?})",
                    dst_path
                )
            }
    ...
    async fn platform_windows(&self, shell: ShellKind) -> Result<RemotePlatform> {
        ...
        Ok(RemotePlatform {
            ...
            arch: match output.trim() {
                ...
                arch => anyhow::bail!(
                    "Prebuilt remote servers are not yet available for windows-{arch}. See https://prism-ide.dev/docs/remote-development"
                ),
            },
        })
    }
