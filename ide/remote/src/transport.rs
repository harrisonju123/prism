...
fn parse_platform(output: &str) -> Result<RemotePlatform> {
    ...
    let os = match os {
        ...
        _ => anyhow::bail!(
            "Prebuilt remote servers are not yet available for {os:?}. See https://prism-ide.dev/docs/remote-development"
        ),
    };
        ...
        || arch.starts_with("aarch64")
    {
        ...
        anyhow::bail!(
            "Prebuilt remote servers are not yet available for {arch:?}. See https://prism-ide.dev/docs/remote-development"
        )
    };
...
async fn build_remote_server_from_source(
    ...
    if let Ok(path) = std::env::var("PRISM_COPY_REMOTE_SERVER") {
        let path = std::path::PathBuf::from(path);
        if path.exists() {
            return Ok(Some(path));
        } else {
            log::warn!(
                "PRISM_COPY_REMOTE_SERVER path does not exist, falling back to ZED_BUILD_REMOTE_SERVER: {}",
                path.display()
            );
        }
    }
    ...
    let build_remote_server =
        std::env::var("PRISM_BUILD_REMOTE_SERVER").unwrap_or("nocompress".into());
    ...
    if platform.os == RemoteOs::Linux && use_musl {
        ...
        if let Ok(path) = std::env::var("PRISM_ZSTD_MUSL_LIB") {
