...
fn validate_release_assets(deps: &[&NamedJob]) -> NamedJob {
    ...
    let validation_script = formatdoc! {r#"
        ...
        ACTUAL_ASSETS=$(gh release view "$TAG" --repo=harrisonju123/PrisM --json assets -q '[.assets[].name]')
...
fn auto_release_preview(deps: &[&NamedJob]) -> NamedJob {
    ...
    named::job(
        dependant_job(deps)
            ...
            .add_step(
                steps::script(
                    r#"gh release edit "$GITHUB_REF_NAME" --repo=harrisonju123/PrisM --draft=false"#,
                )
...
fn upload_release_assets(deps: &[&NamedJob], bundle: &ReleaseBundleJobs) -> NamedJob {
    ...
    named::job(
        dependant_job(&deps)
            ...
            .add_step(
                steps::script("gh release upload \"$GITHUB_REF_NAME\" --repo=harrisonju123/PrisM release-artifacts/*")
...
pub(crate) fn push_release_update_notification(
    ...
    let notification_script = formatdoc! {r#"
        ...
        if [ "$DRAFT_RESULT" == "failure" ]; then
            ...
            RELEASE_URL=$(gh release view "$TAG" --repo=harrisonju123/PrisM --json url -q '.url')
